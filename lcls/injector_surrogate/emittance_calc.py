...
def fit_sigma(sizes, k, axis, sizes_err=None, d=d, l=l, adapt_ranges=False, num_points=5, show_plots=False):
    ...
    if sizes_err is not None and sizes.all()>0 and sizes_err.all()>0:
        ...
        w = None
        abs_sigma = False
    coefs, cov = curve_fit(func, k, sizes**2, sigma=w, absolute_sigma=abs_sigma, method='trf')
...
#             plot_fit(k, sizes, xfit, yerr=sizes_err, axis=axis, save_plot=True, show_plots=show_plots)
    ...
    if np.isnan(coefs).any() or np.isnan(cov).any():
        print('PRINT: RETURNING nans IN EMITTANCE CALC: fit_sigma: 1')
        return np.nan, np.nan, np.nan, np.nan, np.nan 
    ...
    # Constrain emit2 to a non-negative value
    if emit2 < 0:
        emit2 = 1e-20
    ...
    try:
        ...
        print('PRINT: RETURNING nans IN EMITTANCE CALC: fit_sigma: 2')
        return np.nan, np.nan, np.nan, np.nan, np.nan 
...
def get_normemit(energy, xrange, yrange, xrms, yrms, xrms_err=None, yrms_err=None,\
    ...
    if np.isnan(xrms).any() or np.isnan(yrms).any():
        print('PRINT: RETURNING nans IN EMITTANCE CALC: get_normemit: 1')
    ...
    if np.isnan(emitx) or np.isnan(emity):
        ...
        print('PRINT: RETURNING nans IN EMITTANCE CALC: get_normemit: 2')
        return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan
...
def adapt_range(x, y, axis, w=None, fit_coefs=None, x_fit=None, energy=energy, num_points=5, save_plot=False, show_plots=False):
