...
class Opt:
    def __init__(self, init_scan=np.linspace(-6, 0, 10), bsfn=None):
        print("in examples dir")
        self.energy = 0.135
        self.varscan = init_scan
        self.num_points_adapt = 15
        self.pbounds = ((0.46, 0.485), (-0.02, 0.02), (-0.02, 0.02))
        self.plot = False
        self.bsfn = None
    ...
    def run_simplex_opt_norm(self, max_iter):
        ...
        # ref point from sim
        #x0 = [0.47, 0.001499227120199691, -0.0006872989433749197] # results same as refpnt
        ...
        # timestamp = (datetime.datetime.now()).strftime("%Y-%m-%d_%H-%M-%S")
        # np.savetxt(f"simplex_allres_{timestamp}.txt", res, delimiter=',')

        return res
    
    def run_simplex_opt_norm_bounded(self, max_iter):
        np.random.seed(self.seed)
        ...
        bounds = ((self.pbounds[0][0], self.pbounds[0][1]),
                  (self.pbounds[1][0], self.pbounds[1][1]),
                  (self.pbounds[2][0], self.pbounds[2][1])
                )
        res = optimize.minimize(_evaluate, 
                                x0_n, 
                                method='Nelder-Mead', 
                                bounds=bounds,
                                options={
        ...
        timestamp = (datetime.datetime.now()).strftime("%Y-%m-%d_%H-%M-%S")
        np.save(f'simplex_allvecs_{timestamp}.npy', res["allvecs"], allow_pickle=True)
